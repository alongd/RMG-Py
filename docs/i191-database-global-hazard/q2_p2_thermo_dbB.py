#!/usr/bin/env python3
"""
Q2 P2 - separate process. Load DB_B (WITHOUT any thermo library, so methane
falls through to group additivity estimation). Create methane, resolve thermo
via rmgpy.thermo.thermoengine.generate_thermo_data. Record H298 and comment.
...
db = RMGDatabase()
db.load_thermo(settings['database.directory'] + '/thermo', thermo_libraries=[])
print("id(db) =", id(db))
